 ...
 */(function(a,b){
	a.widget("ui.droppable",{
		...
		switch(d){
			case"fit":return i<=e&&f<=j&&k<=g&&h<=l;
			case"intersect":return i<e+b.helperProportions.width/2&&f-b.helperProportions.width/2<j&&k<g+b.helperProportions.height/2&&h-b.helperProportions.height/2<l;
			case"pointer":var m=(b.positionAbs||b.position.absolute).left+(b.clickOffset||b.offset.click).left,n=(b.positionAbs||b.position.absolute).top+(b.clickOffset||b.offset.click).top,o=a.ui.isOver(n,m,k,i,c.proportions.height,c.proportions.width);
			return o;
			case"touch":return(g>=k&&g<=l||h>=k&&h<=l||g<k&&h>l)&&(e>=i&&e<=j||f>=i&&f<=j||e<i&&f>j);
			default:return!1
		}
	}
	,a.ui.ddmanager={
		...
		,prepareOffsets:function(b,c){
			...
			droppablesLoop:for(var g=0;g<d.length;g++){
				...
				e=="mousedown"&&d[g]._activate.call(d[g],c),d[g].offset=d[g].element.offset(),d[g].proportions={
					width:d[g].element[0].offsetWidth,height:d[g].element[0].offsetHeight
				}
			}
		}
		,drop:function(b,c){
	...
	a.widget("ui.resizable",a.ui.mouse,{
		...
		,_mouseStart:function(b){
			...
			this.resizing=!0,this.documentScroll={
				top:a(document).scrollTop(),left:a(document).scrollLeft()
			}
			...
			,this.sizeDiff={
				width:f.outerWidth()-f.width(),height:f.outerHeight()-f.height()
			}
		...
		,_mouseStop:function(b){
			...
			if(this._helper){
				var e=this._proportionallyResizeElements,f=e.length&&/textarea/i.test(e[0].nodeName),g=f&&a.ui.hasScroll(e[0],"left")?0:d.sizeDiff.height,h=f?0:d.sizeDiff.width,i={
					width:d.helper.width()-h,height:d.helper.height()-g
				}
		...
		,_proportionallyResize:function(){
			...
			if(!!this._proportionallyResizeElements.length){
				...
				for(var d=0;d<this._proportionallyResizeElements.length;d++){
					...
					e.css({
						height:c.height()-this.borderDif[0]-this.borderDif[2]||0,width:c.width()-this.borderDif[1]-this.borderDif[3]||0
					})
				}
			}
		}
		...
		,_change:{
			...
			,w:function(a,b,c){
				...
				return{
					left:f.left+b,width:e.width-b
				}
			}
			,n:function(a,b,c){
				var d=this.options,e=this.originalSize,f=this.originalPosition;
				return{
					top:f.top+c,height:e.height-c
				}
			}
			,s:function(a,b,c){
				return{
		...
		stop:function(b,c){
			var d=a(this).data("resizable"),e=d.options,f=d._proportionallyResizeElements,g=f.length&&/textarea/i.test(f[0].nodeName),h=g&&a.ui.hasScroll(f[0],"left")?0:d.sizeDiff.height,i=g?0:d.sizeDiff.width,j={
				width:d.size.width-i,height:d.size.height-h
			}
			...
			if(!!i){
				...
				,e.parentData={
					...
					a(["Top","Right","Left","Bottom"]).each(function(a,b){
						...
						height:j.innerHeight()-k[3],width:j.innerWidth()-k[1]
					};
		...
		,stop:function(b,c){
			...
			d._helper&&!e.animate&&/relative/.test(i.css("position"))&&a(this).css({
				left:k.left-h.left-g.left,width:l,height:m
		...
		start:function(b,c){
			...
			d.ghost=d.originalElement.clone(),d.ghost.css({
				opacity:.25,display:"block",position:"relative",height:f.height,width:f.width,margin:0,left:0,top:0
		...
		,resize:function(b,c){
			...
			d.ghost&&d.ghost.css({
				position:"relative",height:d.size.height,width:d.size.width
			})
		}
		,stop:function(b,c){
			var d=a(this).data("resizable"),e=d.options;
 ...
 */(function(a,b){
	a.widget("ui.sortable",a.ui.mouse,{
		widgetEventPrefix:"sort",ready:!1,options:{
			...
			if(!!b){
				...
				if(this.options.revert){
					...
					d.reverting=!0,a(this.helper).animate({
						left:e.left-this.offset.parent.left-d.margins.left+(this.offsetParent[0]==document.body?0:this.offsetParent[0].scrollLeft),top:e.top-this.offset.parent.top-d.margins.top+(this.offsetParent[0]==document.body?0:this.offsetParent[0].scrollTop)
					}
 ...
 */(function(a,b){
	a.widget("ui.accordion",{
		options:{
			active:0,animated:"slide",autoHeight:!0,clearStyle:!1,collapsible:!1,event:"click",fillSpace:!1,header:"> li > :first-child,> :not(li):even",icons:{
		...
		,_keydown:function(b){
			if(!(this.options.disabled||b.altKey||b.ctrlKey)){
				...
				switch(b.keyCode){
					case c.RIGHT:case c.DOWN:f=this.headers[(e+1)%d];
					break;
					case c.LEFT:case c.UP:f=this.headers[(e-1+d)%d];
					break;
					case c.SPACE:case c.ENTER:this._clickHandler({
						target:b.target
					}
 ...
 */(function(a,b){
	...
	a.widget("ui.autocomplete",{
		options:{
			appendTo:"body",autoFocus:!1,delay:300,minLength:1,position:{
		...
		,pending:0,_create:function(){
			...
			this.element.addClass("ui-autocomplete-input").attr("autocomplete","off").attr({
				...
				if(!b.options.disabled&&!b.element.propAttr("readOnly")){
					...
					var e=a.ui.keyCode;
					switch(c.keyCode){
						case e.PAGE_UP:b._move("previousPage",c);
						break;
						case e.PAGE_DOWN:b._move("nextPage",c);
						break;
						case e.UP:b._move("previous",c),c.preventDefault();
						break;
						case e.DOWN:b._move("next",c),c.preventDefault();
						break;
						case e.ENTER:case e.NUMPAD_ENTER:b.menu.active&&(d=!0,c.preventDefault());
						case e.TAB:if(!b.menu.active)return;
						b.menu.select(c);
						break;
						case e.ESCAPE:b.element.val(b.term),b.close(c);
						break;
		...
		,_initSource:function(){
			...
			a.isArray(this.options.source)?(d=this.options.source,this.source=function(b,c){
				...
				b.xhr&&b.xhr.abort(),b.xhr=a.ajax({
					url:e,data:d,dataType:"json",context:{
 ...
 */(function(a,b){
	...
	a.widget("ui.button",{
		options:{
			disabled:null,text:!0,label:null,icons:{
 ...
 */(function(a,b){
	...
	a.widget("ui.dialog",{
		options:{
			autoOpen:!0,buttons:{},closeOnEscape:!0,closeText:"close",dialogClass:"",draggable:!0,hide:null,height:"auto",maxHeight:!1,maxWidth:!1,minHeight:150,minWidth:150,modal:!1,position:{
				my:"center",at:"center",collision:"fit",using:function(b){
					var c=a(this).css(b).offset().top;
					c<0&&a(this).css("top",b.top-c)
				}
			}
			,resizable:!0,show:null,stack:!0,title:"",width:740,height:549,zIndex:1e3
		}
		,_create:function(){
			...
			var b=this,d=b.options,e=d.title||"&#160;",f=a.ui.dialog.getTitleId(b.element),g=(b.uiDialog=a("<div></div>")).appendTo(document.body).hide().addClass(c+d.dialogClass).css({
				zIndex:d.zIndex
			}).attr("tabIndex",-1).attr("id","uii").css("outline",0).keydown(function(c){/***************************************/
		...
		,moveToTop:function(b,c){
			...
			e.zIndex>a.ui.dialog.maxZ&&(a.ui.dialog.maxZ=e.zIndex),d.overlay&&(a.ui.dialog.maxZ+=1,d.overlay.$el.css("z-index",a.ui.dialog.overlay.maxZ=a.ui.dialog.maxZ)),f={
				scrollTop:d.element.scrollTop(),scrollLeft:d.element.scrollLeft()
			}
		...
		,_makeDraggable:function(){
			function f(a){
				return{
					position:a.position,offset:a.offset
				}
			}
			var b=this,c=b.options,d=a(document),e;
			b.uiDialog.draggable({
				cancel:".ui-dialog-content, .ui-dialog-titlebar-close",handle:".ui-dialog-titlebar",containment:"document",start:function(d,g){
		...
		,_makeResizable:function(c){
			...
			d.uiDialog.resizable({
				cancel:".ui-dialog-content",containment:"document",alsoResize:d.element,maxWidth:e.maxWidth,maxHeight:e.maxHeight,minWidth:e.minWidth,minHeight:d._minHeight(),handles:g,start:function(b,c){
		...
		,_position:function(b){
			...
			if(b){
				if(typeof b=="string"||typeof b=="object"&&"0"in b)c=b.split?b.split(" "):[b[0],b[1]],c.length===1&&(c[1]=c[0]),a.each(["left","top"],function(a,b){
					...
					my:c.join(" "),at:c.join(" "),offset:d.join(" ")
				};
		...
		,_setOption:function(b,d){
			...
			switch(b){
				case"beforeclose":b="beforeClose";
				break;
				case"buttons":e._createButtons(d);
				break;
				case"closeText":e.uiDialogTitlebarCloseText.text(""+d);
				break;
				case"dialogClass":f.removeClass(e.options.dialogClass).addClass(c+d);
				break;
				case"disabled":d?f.addClass("ui-dialog-disabled"):f.removeClass("ui-dialog-disabled");
				break;
				case"draggable":var g=f.is(":data(draggable)");
				g&&!d&&f.draggable("destroy"),!g&&d&&e._makeDraggable();
				break;
				case"position":e._position(d);
				break;
				case"resizable":var h=f.is(":data(resizable)");
				h&&!d&&f.resizable("destroy"),h&&typeof d=="string"&&f.resizable("option","handles",d),!h&&d!==!1&&e._makeResizable(d);
				break;
				case"title":a(".ui-dialog-title",e.uiDialogTitlebar).html(""+(d||"&#160;"))
			}
		...
		,_size:function(){
			...
			this.element.show().css({
				...
				width:this.options.width||"auto",minHeight:0,height:0
			}),b.minWidth>b.width&&(b.width=b.minWidth),c=this.uiDialog.css({
				height:"auto",width:b.width
			}).height(),d=Math.max(0,b.minHeight-c);
			if(b.height==="auto")if(a.support.minHeight)this.element.css({
				minHeight:d,height:"auto"
			});
			else{
				this.uiDialog.show();
		...
		instances:[],oldInstances:[],maxZ:0,events:a.map("focus,mousedown,mouseup,keydown,keypress,click".split(","),function(a){
		...
		,resize:function(){
			...
			a.each(a.ui.dialog.overlay.instances,function(){
				...
				width:a.ui.dialog.overlay.width(),height:a.ui.dialog.overlay.height()
			})
		}
 ...
 */(function(a,b){
	...
	a.widget("ui.slider",a.ui.mouse,{
		...
		,_create:function(){
			...
			,function(){
				...
				if(!b.options.disabled){
					switch(d.keyCode){
						case a.ui.keyCode.HOME:case a.ui.keyCode.END:case a.ui.keyCode.PAGE_UP:case a.ui.keyCode.PAGE_DOWN:case a.ui.keyCode.UP:case a.ui.keyCode.RIGHT:case a.ui.keyCode.DOWN:case a.ui.keyCode.LEFT:d.preventDefault();
						if(!b._keySliding){
		...
		,_setOption:function(b,c){
			...
			switch(b){
				case"disabled":c?(this.handles.filter(".ui-state-focus").blur(),this.handles.removeClass("ui-state-hover"),this.handles.propAttr("disabled",!0),this.element.addClass("ui-disabled")):(this.handles.propAttr("disabled",!1),this.element.removeClass("ui-disabled"));
				break;
 ...
 */(function(a,b){
	...
	a.widget("ui.tabs",{
		...
		,_ui:function(a,b){
			return{
				tab:a,panel:b,index:this.anchors.index(a)
			}
		}
		,_cleanup:function(){
		...
		rotation:null,rotate:function(a,b){
	...
	$.extend(Datepicker.prototype,{
		...
		,_doKeyDown:function(a){
			...
			if($.datepicker._datepickerShowing)switch(a.keyCode){
				case 9:$.datepicker._hideDatepicker(),c=!1;
				break;
		...
		,parseDate:function(a,b,c){
			...
			for(var s=0;s<a.length;s++)if(m)a.charAt(s)=="'"&&!n("'")?m=!1:q();
				...
				break;
				case"!":var t=new Date((o("!")-this._ticksTo1970)/1e4);
				i=t.getFullYear(),j=t.getMonth()+1,k=t.getDate();
				break;
				case"'":n("'")?q():m=!0;
				break;
		...
		,ATOM:"yy-mm-dd",COOKIE:"D, dd M yy",ISO_8601:"yy-mm-dd",RFC_822:"D, d M y",RFC_850:"DD, dd-M-y",RFC_1036:"D, d M y",RFC_1123:"D, d M yy",RFC_2822:"D, d M yy",RSS:"D, d M y",TICKS:"!",TIMESTAMP:"@",W3C:"yy-mm-dd",_ticksTo1970:(718685+Math.floor(492.5)-Math.floor(19.7)+Math.floor(4.925))*24*60*60*1e7,formatDate:function(a,b,c){
			...
			if(b)for(var m=0;m<a.length;m++)if(l)a.charAt(m)=="'"&&!h("'")?l=!1:k+=a.charAt(m);
				...
				case"d":k+=i("d",b.getDate(),2);
				break;
				case"D":k+=j("D",b.getDay(),d,e);
				break;
				case"o":k+=i("o",Math.round(((new Date(b.getFullYear(),b.getMonth(),b.getDate())).getTime()-(new Date(b.getFullYear(),0,0)).getTime())/864e5),3);
				break;
				...
				case"@":k+=b.getTime();
				break;
				case"!":k+=b.getTime()*1e4+this._ticksTo1970;
				break;
				case"'":h("'")?k+="'":l=!0;
				break;
		...
		,_possibleChars:function(a){
			...
			for(var e=0;e<a.length;e++)if(c)a.charAt(e)=="'"&&!d("'")?c=!1:b+=a.charAt(e);
				...
				case"d":case"m":case"y":case"@":b+="0123456789";
				break;
		...
		,_determineDate:function(a,b,c){
			...
			,e=function(b){
				...
				while(i){
					switch(i[2]||"d"){
						case"d":case"D":g+=parseInt(i[1],10);
						break;
			...
			for(var L=0;L<g[0];L++){
				...
				for(var N=0;N<g[1];N++){
					...
					if(j){
						...
						if(g[1]>1)switch(N){
							case 0:Q+=" ui-datepicker-group-first",P=" ui-corner-"+(c?"right":"left");
							break;
	...
	a.effects.animateClass=function(b,c,d,e){
		...
		return this.queue(function(){
			...
			a.each(f,function(a,c){
				...
				queue:!1,duration:c,easing:d,complete:function(){
					a.each(f,function(a,c){
						b[c]&&g[c+"Class"](b[c])
	...
	,a.fn.extend({
		...
		,getBaseline:function(a,b){
			...
			switch(a[0]){
				...
				case"bottom":c=1;
				break;
				default:c=a[0]/b.height}switch(a[1]){case"left":d=0;break;case"center":d=.5;break;case"right":d=1;break;default:d=a[1]/b.width
			}
			return{
				x:d,y:c
			}
		...
		,createWrapper:function(b){
			...
			var c={
				width:b.outerWidth(!0),height:b.outerHeight(!0),"float":b.css("float")
			}
			...
			b.wrap(d),(b[0]===e||a.contains(b[0],e))&&a(e).focus(),d=b.parent(),b.css("position")=="static"?(d.css({
				...
				position:b.css("position"),zIndex:b.css("z-index")
		...
		effect:function(b,c,d,e){
			var f=k.apply(this,arguments),g={
				options:f[1],duration:f[2],callback:f[3]
			}
	...
	a.effects.clip=function(b){
		return this.queue(function(){
			...
			var g=a.effects.createWrapper(c).css({
				...
				size:f=="vertical"?"height":"width",position:f=="vertical"?"top":"left"
			}
	...
	a.effects.explode=function(b){
		return this.queue(function(){
			...
			for(var i=0;i<c;i++)for(var j=0;j<d;j++)e.clone().appendTo("body").wrap("<div></div>").css({
				...
				left:f.left+j*(g/d)+(b.options.mode=="show"?0:(j-Math.floor(d/2))*(g/d)),top:f.top+i*(h/c)+(b.options.mode=="show"?0:(i-Math.floor(c/2))*(h/c)),opacity:b.options.mode=="show"?1:0
			}
			,b.duration||500);
			setTimeout(function(){
	...
	a.effects.highlight=function(b){
		return this.queue(function(){
			...
			e=="hide"&&(f.opacity=0),a.effects.save(c,d),c.show().css({
				backgroundImage:"none",backgroundColor:b.options.color||"#ffff99"
			}).animate(f,{
				queue:!1,duration:b.duration,easing:b.options.easing,complete:function(){
 ...
 */(function(a,b){
	a.effects.puff=function(b){
		return this.queue(function(){
			...
			a.extend(b.options,{
				fade:!0,mode:d,percent:d=="hide"?e:100,from:d=="hide"?g:{
	...
	,a.effects.scale=function(b){
		return this.queue(function(){
			...
			:i);
			var j={
				y:g!="horizontal"?f/100:1,x:g!="vertical"?f/100:1
			};
			c.to={
				height:i.height*j.y,width:i.width*j.x
			}
	...
	,a.effects.size=function(b){
		return this.queue(function(){
			...
			var q={
				from:{
					y:c.from.height/n.height,x:c.from.width/n.width
				}
				,to:{
					y:c.to.height/n.height,x:c.to.width/n.width
				}
			};
			...
			if(l=="content"||l=="both")h=h.concat(["marginTop","marginBottom"]).concat(g),i=i.concat(["marginLeft","marginRight"]),f=d.concat(h).concat(i),c.find("*[width]").each(function(){
				...
				child.from={
					height:c.height*q.from.y,width:c.width*q.from.x
				}
				,child.to={
					height:c.height*q.to.y,width:c.width*q.to.x
				}
